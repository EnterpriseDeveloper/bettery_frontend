...
export class ErcCoinSaleComponent implements OnInit, OnDestroy {
  ...
  async connectToMetamask() {
    ...
    if (!(window as any).ethereum) {
      this.openMetamaskModal(
        "Please install Metamask in your browser to proceed.",
        "Metamask required!",
      );
      this.spinner = false;
    } else {
      if (!this.web3) {
        try {
          ...
          this.openMetamaskModal(
            "Please allow Metamask to access to the webpage.",
            "access needed",
          );
          this.spinner = false;
        }
      ...
      if (!coinbase) {
        this.openMetamaskModal(
          "Please activate MetaMask first.",
          "access needed",
        );
        this.spinner = false;
        return;
        ...
        if (checkNetwork != "5") {
          this.openMetamaskModal(
            "Plaese switch your network in MetaMask to the Goerli network.",
            "access needed",
          );
          this.spinner = false;
        } else {
  ...
  async buyToken(wallet) {
    // TODO: implement token sale
    // try {
    //   let networkId = 5;
    //   let tokenSaleAddress = TokenSale.networks[networkId].address;
    //   let tokenAddress = "0xFCf9F99D135D8a78ab60CC59CcCF3108E813bA35";
    //   let number = (
    //     Number(this.numberOfTokens) * Number(this.tokensaleInfo.price)
    //   ).toFixed(4);
    //   let usdtContract = await this.connectToContract(
    //     wallet,
    //     USDTToken,
    //     tokenAddress,
    //   );
    //   let approveAmount = this.web3.utils.toWei(String(number), "mwei");
    //   let approve = await usdtContract.methods
    //     .approve(tokenSaleAddress, approveAmount)
    //     .send();
    //   console.log(approve);
    //   let tokenSaleContract = await this.connectToContract(
    //     wallet,
    //     TokenSale.abi,
    //     tokenSaleAddress,
    //   );
    //   let tokensAmount = this.web3.utils.toWei(
    //     String(this.numberOfTokens),
    //     "ether",
    //   );
  ...
  openMetamaskModal(message, title) {
    const modalRef = this.modalService.open(MetaMaskModalComponent, {
      centered: true,
    });
  ...
  subscribe() {
    ...
    this.subscribedPost = this.postService.post("subscribe", data).subscribe(
      (x) => {
