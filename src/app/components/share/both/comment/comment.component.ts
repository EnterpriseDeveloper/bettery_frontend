...
export class CommentComponent implements OnInit, OnDestroy, OnChanges {
  ...
  activated = ["active1", "active2", "active3"];
  timeOutTyping: any;
  isReply = {
    isReply: false,
    commentId: null,
  ...
  constructor(
    ...
    this.showLength =
      this.allComments?.length < 10 ? this.allComments?.length : 10;
  }
  ...
  initializeSocket(id): void {
    ...
    this.newCommentSub = this.socketService
      .newComment()
  ...
  async sendComment() {
    if (this.userData?._id === undefined) {
      ...
      const el = this.document.getElementById("textarea");
  ...
  sendIcon(text: string, commentId: number) {
    if (this.userData && text && commentId) {
      this.socketService.sendSelectedIcon(
        text,
        this.id,
        this.userData._id,
        commentId,
  ...
  finishScrollAnimation(event: boolean, id) {
    ...
    const styleStart =
      "background-color: rgba(68, 68, 68, 0.7); border-radius: 8px; transition: all 200ms;";
